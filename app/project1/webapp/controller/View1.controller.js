    ...
    return Controller.extend("project1.controller.View1", {
        onInit: function () {
         ...
         onValidateStep1: function () {
            const oWizard = this.byId("approvalWizard");
            const oStep1 = this.byId("step1");
            const sIssueId = this.byId("issueIdInput").getValue();

            if (sIssueId) {
                oWizard.validateStep(oStep1);
                this.byId("reviewIssueId").setText(sIssueId);
            } else {
        ...
        onSubmit: function () {
            const issueId = this.byId("issueIdInput").getValue();

            if (!issueId) {
                MessageToast.show("Please enter Issue ID");
                return;
            }

            // Call CAP action here
            MessageToast.show("Approval sent for Issue ID: " + issueId);
        },
